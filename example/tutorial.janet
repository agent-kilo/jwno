...
(defn build-slide-keymap []
  (let [keymap (:new-keymap key-man common-keymap)]
    (:define-key keymap
                 "win + enter"
                 [:show-keymap keymap]
                 "Show current keymap")
...
(defn intro
  ...
  (:show-tooltip
     ...
     (format-slide-text
      ...
      Jwno is a keyboard-centric tiling window manager, we'll be using
      your keyboard a lot, so keep it handy ;)

      Note that Jwno has no key bindings defined by default, the keys
      we'll use here are specifically defined for this tutorial.

      Press Space to continue.
      Press Win + Q or right click on Jwno's system tray icon at any
      time to stop this tutorial and exit.
      ```)
     nil nil
     0
     :center))
...
(defn before-cleanup-desktop
  ...
  (:show-tooltip
     ...
     (format-slide-text
      ...
      To do that, I'll first try to minimize all your windows to make us
      a clean desktop. You can also manually close them before we continue.
  ...
  (ev/spawn
   ...
   (:show-tooltip
      ...
      (format-slide-text
       id total
       ```
       Now that we have our windows in place, let's tile them.
...
(defn tile-windows-in-top-frame
  ...
  (:show-tooltip
     ...
     (format-slide-text
      id total
      ```
      Jwno calls the screen areas it manages "frames." It will
      allocate a top-level frame that covers the whole desktop
      area for each monitor by default.

      The area now occupied by the Notepad window is our top
      frame for this monitor.

      Note that Jwno will show a gap of 20 virtual pixels
      around the windows in this tutorial, so that we can
      easily visualize frame borders.
...
(defn split-top-frame
  ...
  (:call-command command-man :split-frame :horizontal nil nil
                 (fn [fr]
  ...
  (:show-tooltip
     ...
     (format-slide-text
      ...
      We just called the :split-frame command and have the
      top frame split into two child frames. This is called
      a "horizontal split," since after the split, child
      frames are lined up horizontally.

      Don't worry about the colon(:) in front of the command
      name. Just knowing that some names in Jwno start with
      a colon is sufficient for this tutorial.
...
(defn split-more-frames
  ...
  (:call-command command-man :split-frame :vertical nil nil
                 (fn [fr]
                   (def w (:get-current-window fr))
                   (when w
                     (:add-child (get-in fr [:children 1]) w))))
...
(defn about-active-frames
  ...
  (:show-tooltip
     ...
     (format-slide-text
      id total
      ```
      Jwno has the concept of "active frames." An active frame
      is a frame that currently has the input focus. New windows
      will snap into the active frame, and all frame selection
      commands are relative to the active frame.
      ...
      You can click on the Notepad windows to observe the switching
      of active frames. There will be a small tooltip appearing in
      the center of the activated frame.
...
(defn split-even-more-frames
  ...
  (:show-tooltip
     ...
     (format-slide-text
      id total
      ```
      And we just called the :split-frame command and did a
...
(defn about-empty-frames
  ...
  (:show-tooltip
     ...
     (format-slide-text
      ...
      And if we open a new window...
      (I'll spawn an Explorer window for you.)
...
(defn about-enforcing-window-geometries
  ...
  (:show-tooltip
     ...
     (format-slide-text
      id total
      ```
      But Jwno does not enforce window geometries. Instead,
      it only resizes and relocates windows in these situations:
      ...
      You can now freely move or resize any managed windows
      on the screen. I'll retile them for you in the next step.
...
(defn about-retiling
  ...
  (:show-tooltip
     ...
     (format-slide-text
      ...
      We just called the :retile command. When you temporarily
      altered the layout of your windows, or want to deal with
      some weird windows that did not snap to the frames cleanly,
...
(defn show-move-window-command-filter [cmd args]
  (when (= cmd :move-window)
    (:show-tooltip
       ui-man
       :move-window-command
       (string/format "[%n %n]" cmd (first args))
       nil nil
       2000
       :top-left))
  true)
...
(defn about-moving-windows
  ...
  (:add-hook hook-man :filter-command show-move-window-command-filter)

  (each dir [:left :up :right :down]
    (:define-key keymap (string dir) [:move-window dir]))
  (:set-keymap key-man keymap)
  ...
  (:show-tooltip
     ...
     (format-slide-text
      ...
      Select a window, and try the arrow keys on your keyboard ;)
      The arrow keys are now bound to window movement commands.
      The command that is actually called will be shown briefly
      in the top-left corner of your current monitor.
...
(defn about-moving-windows-across-monitors
  {:slide 15}
  [id total keymap]

  (:remove-hook hook-man :filter-command show-move-window-command-filter)
  ...
  (def text
    (if (> (length (in layout :children)) 1)
      ```
      I see that you have multiple monitors. Jwno has exellent
      multi-monitor support, and knows about your monitor arrangement,
      so the window movement commands also work across different
      monitors, as you may have noticed.

      In other words, you can generally treat your monitors as
      normal frames.
      ```
      # else
      ```
      Jwno has exellent multi-monitor support, and knows about
      your monitor arrangement. So if you have multiple monitors,
      the window movement commands would also work across all
      your monitors.
