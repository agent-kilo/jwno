...
(defn test-keymap-parse-key []
  ...
  (assert (= (:parse-key keymap "win-a")
             (key (ascii "A") [:win])))
  (assert (= (:parse-key keymap "lwin+a")
             (key (ascii "A") [:lwin])))
  (assert (= (:parse-key keymap "lwin+rwin+a")
             (key (ascii "A") [:lwin :rwin])))
  (assert (= (:parse-key keymap "win+ctrl+a")
             (key (ascii "A") [:ctrl :win])))
  (assert (= (:parse-key keymap "win+ctrl+shift+a")
             (key (ascii "A") [:ctrl :win :shift])))
  (assert (= (:parse-key keymap "win+ctrl+shift+alt+a")
             (key (ascii "A") [:ctrl :win :shift :alt])))
  (assert (= (:parse-key keymap "win+enter")
             (key VK_RETURN [:win])))
  (assert (= (:parse-key keymap "win+ctrl+enter")
  ...
  (assert (= (:parse-key keymap "win+eNTER")
             (key VK_RETURN [:win])))
  (assert (= (:parse-key keymap "win+f1")
             (key VK_F1 [:win])))
  (assert (= (:parse-key keymap "win+F1")
             (key VK_F1 [:win])))
  (assert (= (:parse-key keymap "win+f10")
             (key VK_F10 [:win])))
  (assert (= (:parse-key keymap "win+f19")
             (key VK_F19 [:win])))
  (assert (= (:parse-key keymap "win+f20")
             (key VK_F20 [:win])))
  (assert (= (:parse-key keymap "win+f24")
             (key VK_F24 [:win])))
